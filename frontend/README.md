...
- `public/models/mountain-demo/` 是项目自有的离线山地 GLB、场景配置、点位清单与来源说明；
  `tools/mountain_asset/generate_mountain_glb.py` 可无 Blender、无网络重新生成。
...
本地模型以锚点 `113.0508°E, 23.7208°N, 2m` 建立 ENU（东-北-上）局部坐标系；GLB 的
`+X/+Y/+Z` 分别对应东/北/上。V9 迁移中的经纬度和高程与 `points.json` 同源，因此立柱直接
落在 GLB 坡面上。Cesium 的 `CLAMP_TO_GROUND` 只能贴椭球/terrain，不能贴独立 GLB，
所以山地模式不使用地形钳制。切到 `globe` 模式且真实地形就绪后，仍会采样在线地形高度。
...
| 12 | 个人中心（`/profile`） | ✅ 2026-09-17 晚：顶栏头像下拉进入；只读资料（姓名/账号/角色/公司/岗位/电话/邮箱）+ 修改密码（验原密码、新密码 ≥8 位、两次一致）；改密后当前页继续可用、其它端全部下线。同轮 `selfcheck` 67 → **73 条** |
| 13 | 管理端「用户」页签 | ✅ 2026-09-17 晚：账号列表（账号/姓名/角色/公司/岗位/电话/邮箱/启用）+ 新建（初始密码 ≥8 位）+ 改资料/角色/启用；用户页不出删除按钮（账号用停用，删除留给建错的号）。同轮修掉一个**页签切换的竞态**：`reload()` 原先读的是「响应到达那一刻」的 `currentTab`，切页签后会把上一个页签的数据套在新列上渲染（实测表现为「用户页 2 行、每格都是 —」），现改为**先快照页签 + 代际守卫**，旧响应一律丢弃 |
...
- **个人中心的资料是只读的，也没有自助注册**（2026-09-17 晚）：公司（= 账号所属组织名）/
  岗位 / 电话 / 邮箱展示自后端 `sys_user`（V17），但**只有登录用户自己能看、不能自己改**——
  这些是组织属性，本人可改等于「谁是谁」可以自证；修改渠道是管理员，而**管理员的用户管理页
  （建号 / 停用 / 重置密码 / 分配项目）还没做**（改进清单第 13 条）。登录页也**没有注册入口**：
  演示账号由后端 `DataInitializer` 建，生产 profile 下连它也默认关闭。
