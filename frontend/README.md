...
### 系统运维页 `/ops`（2026-09-18，仅 ADMIN）

与 `/admin` 的分工写在 `views/OpsView.vue` 顶部：**`/admin` 管数据，`/ops` 看服务**。
页面读四个只读端点（`/api/v1/ops/{status,migrations,stats,config}`）：

- 服务状态：进程、profile、JVM 堆、可用核数、SSE 连接数、数据库产品与连接串（脱敏）、连接池；
- 数据库迁移：Flyway 清单（版本/脚本/校验和/状态/执行时间/耗时），当前版本与待执行条数；
- 数据规模：关键表行数、测量值实时/补报条数、时间跨度、未解除警情；
- 运行配置：严格契约、批大小、时钟容差、上传目录、演示账号开关、令牌有效期、接入密钥（只回显长度与"是否默认值"）。

页面顶部还有「接口文档」按钮——它指向后端自带的 Swagger，`nginx.conf` 里为此补了
`/swagger-ui/`、`/v3/api-docs`、`/webjars/` 三条反代（不补就是 404）。

### 使用说明页 `/help`（2026-09-18，所有角色）

侧边栏**最后一项**。内容以「当前登录角色」开头（四个角色的落地页、可见菜单、可做的处置动作
不同，见 `AlarmConstants.ROLE_ACTIONS`），其后按侧边栏顺序逐个介绍模块的**功能 + 注意事项**，
最后是 7 条常见口径（时间、数据范围、告警判定、逻辑删除、权限边界、主题、数据性质声明）。

写这一页的两条约定：① 它是**界面口径的复述**，阈值/判据这类会漂移的数字不抄进来，
只说"见告警规则"；② 角色差异不写死成四份文案，而是按 `ROLE_INFO[userStore.role]` 取当前角色，
角色表本身是唯一数据源。
