...
- **2026-09-23 第四轮（最新）**：标签**屏幕空间避让**（测点给分区名让位）+ 位移引线改走
  2D 叠加层（3D 折线会被山体整段遮掉）+ 远景补 z16/z17 + **后端日志 debug→info**
  （这条把大屏首屏批量接口从 24 s 拉回毫秒级，之前一直顶着"加载失败"红灯）。
  **接手请先读 `docs/进度交接_20260923_第四轮.md`**。
- 2026-09-23 第三轮（标签随缩放自适应 / 相机限制放开 / 远景白瓦片与缓存击穿 /
  数据面板动效，另外修掉三个真 bug：配置加载竞态、`pin` 未清理、并发建场景互删）：
  细节与"服务怎么起、当前生效开关"见 `docs/进度交接_20260923.md`。
