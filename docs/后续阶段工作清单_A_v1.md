...
>
> **更新（2026-09-10 深夜）**：本机装好 Docker，**B-4（PG 迁移验证）已验完**——`postgres:16` 上 V1–V4 全部迁移成功、**173 条断言 173/173**、重启后端数据不丢。「本机无 Docker/PG」这条老卡点至此注销。**M0 签字这步用户定案：先不动，等前端动工前再收口**——理由是 B 转前端后契约实际由 A 单方维护，签字此时已无实质约束力；等 B 真照契约写前端时签一次才有意义。届时需要的不是「签字仪式」，而是先把 `M0_接口冻结_致B_v1.md`（停在 09-09）与现状的几处矛盾抹平（详见 §5）。
...
| ~~本机无 Docker/PG~~ | ✅ **已解除** | Docker 已装（走清华源 + 镜像加速器）；B-4 已在 `postgres:16` 上验完（173/173 + 重启不丢数据）。B-3 的 compose 编排仍未做——本次是手工 `docker run` 起的库 |
| `M0_接口冻结_致B_v1.md` 与现状不符 | 前端动工前签字会签「一份已经不是真的文件」 | 该文停在 09-09，**D5 仍写 `THRESHOLD/RATE/CHANGE` 三者合法**（实际只收 `THRESHOLD`，传 RATE/CHANGE → 400）、**D6 仍写默认规则 `gte 10/恢复 5`**（实际三条：±3.0 warning + +5.0 alarm）、**D2 写「不需要改表」**（已到 V3/V4）；且 D6 在那份文件内部就自相矛盾（§2 写 10、§6 写 ±3）。<br>**维护中、且内容正确的是** `docs/B侧接口契约_M0.md`（09-10 已同步 THRESHOLD-only、三条种子规则、collectTime 拒收、时间带时区）。<br>→ 收口时二选一：A 出「签署版」刷新 D1–D10，或直接以 `B侧接口契约_M0.md` 为签署对象。<br>（另：`message-contract.md` 缺 `collectTime` 非法即 REJECTED 这条——它只写在 `B侧接口契约_M0.md §2`，而前者自称「M0 唯一事实源」，建议补一句指向。） |
