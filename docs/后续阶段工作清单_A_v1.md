...
- **第 8 条**（docker compose 一键 + 数据持久）：**A（A5）** → ✅ **已落地（B-3）**。「一键」与「数据持久」两句都实测过了。**「浏览器完整可访问」**：前端本身已可访问（`npm run dev` → `localhost:5173`，2026-09-11 浏览器实测），但**构建产物还没进 compose**——`docker-compose.yml` 末尾那段 `frontend` 服务仍是注释块，补上即可让「一键」覆盖到浏览器这一面。
