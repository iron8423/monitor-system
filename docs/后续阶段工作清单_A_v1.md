...
但**尚未到「阶段 1 整体完成」**：M0 契约差 B 签字（Q1–Q3 未答 → D1/D2 悬着），
且后端闭环验收（M2：超限→告警→处置→解除）依赖 B 的 ingest/模拟器/告警模块。
另：git 尚无首个提交。
...
| # | 事项 | 阻塞 | 说明 |
|---|---|---|---|
| A-1 | **首个 git commit**（A 交付基线） | 无 | 81 文件已 add 未提交。建议 `feat(repo): 骨架+A0–A4+M0（含 daily）`；B 才能拉分支协同（§5.2） |
| A-2 | **M0 冻结收口**：发 B《M0_接口冻结_致B_v1》，收 Q1–Q3 答复 | 等 B | B 答复后 A 一天内完成收尾 |
| A-3 | **D1/D2 收尾**（V2 测项种子/默认规则/契约对齐） | 等 B Q1–Q3 | 默认口径 `defo_mm/rate_mm_d`；B 若含三分量则保留分量测项。落地即改 V2 + message-contract §2 |
...
| 卡点 | 影响 | 对策 |
|---|---|---|
| git 无首个提交 | B 无法拉分支并行 | A-1 今天做首个 commit |
| M0 未签字，契约不同源（defo_mm vs X/Y/Z/DISP/VEL） | D1/D2 悬着、V2 别大动 | A-2/A-3 等 B Q1–Q3；签字前不改 V2 大结构 |
