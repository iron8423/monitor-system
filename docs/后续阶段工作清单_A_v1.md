...
| A-7 | **验收套件落仓** | ✅ 已完成 | `tools/acceptance/`：**8 套件 / 188 条断言**（2026-09-11：173 → 175 加 `03-query.sh` ⑨；175 → 188 加 `04-alarm.sh` ⑪ 的 13 条角色越权断言）+ `run-all.sh --fresh`（另起空库后端）+ README（脚本↔§9 验收条映射）。覆盖 §9 里后端可独立验证的部分。对应 M5 判据「一键过验收脚本」的后端部分。 |
...
| B-13 | **点号长度在两处不一致，33–64 字符的测点永远上不了数** | ⚠️ **待用户/B 定案，本轮未修**（2026-09-11 发现） | `monitor_point.code` 是 **VARCHAR(64)**（管理端建档照收），但 `monitor_measurement.point_code` 只有 **VARCHAR(32)**（上报时冗余写入）。于是点号 33–64 字符的测点**建得出来、列表看得见、永远收不到数据也永远不产生警情**，一上报即 `HTTP 500`（后端日志 `Value too long for column "POINT_CODE CHARACTER VARYING(32)"`）。两处本该一致——该列注释自己写着「冗余业务键（上报即此码）」。<br>**发现路径**：写角色矩阵脚本时点号起长了（`P-ARM-admin-confirm-<RUN_ID>` = 37 字符）撞出来的；验收套件一直没抓到，是因为它用的临时点号都在 32 以内（`P-ALM-<RUN_ID>` = 26）。<br>**建议改法**：加 Flyway **V5** 把 `monitor_measurement.point_code` 放宽到 64（另一条路是把 `monitor_point.code` 收到 32，但那会否决已在用的长点号，方向更可能是前者）。**三个配套项别漏**：① `docs/B侧接口契约_M0.md` 要同步注明点号长度上限；② 建档接口宜加长度校验，把「能建不能收」变成「建的时候就报 400」；③ `measurement_point_code` 上若有索引需重建。<br>**为什么本轮不修**：属 B 的 schema 与契约范围（`docs/B侧接口契约_M0.md`），且改列宽要动迁移脚本——A 的唯一维护权只覆盖「加表/加列走 A 统一出 V3」这种协作约定，不宜单方面改已冻结的结构。 |
...
| M5 演示就绪 | 阶段 2 · 第 14 天 | 影像挂点+实时；A：Docker Compose | ✅ A5 已完成（B-3，2026-09-10） | ✅ `docker compose up` 起 **db + backend + frontend** 三个服务，`run-all.sh` 一键过验收脚本（2026-09-11 最新：`--fresh` 空库 **188/188**；compose PG 形态 04 套件 46/46）；前端构建产物**已进 compose**，浏览器端到端在同一形态实测通过 |
