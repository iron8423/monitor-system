...
| M5 演示就绪 | 阶段 2 · 第 14 天 | 影像挂点+实时；A：Docker Compose | ✅ A5 已完成（B-3，2026-09-10） | ✅ `docker compose up` 起 **db + backend + frontend** 三个服务，`run-all.sh` 一键过验收脚本（2026-09-11 复跑 **175/175**）；前端构建产物**已进 compose**，浏览器端到端在同一形态实测通过 |
