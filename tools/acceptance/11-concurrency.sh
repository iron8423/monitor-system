...
# 两者都正确，取决于请求到达的先后；但**至少有一个 409** 才说明 CAS 真的在起作用——
# 全是 400 的话，可能是所有请求都被串行化了，那 ② 就没测到并发路径。
CTS=$(count_codes '409')
if [ "$CTS" -ge 1 ]; then
  pass "有 $CTS 个请求走了 CAS 落空路径（409）"
else
  fail "没有任何请求走 CAS 落空路径（409）" \
       "全是 400 说明请求被串行化了，本节的并发断言没有覆盖到真正的并发路径；状态码分布：$(grep -h . "$TMP"/code* | sort | uniq -c | tr '\n' ' ')"
fi
