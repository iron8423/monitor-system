...
# 22-line-of-sight 也属于「造数并自己回收」这一类（两个临时测点 + 一次故意被拒的标定），
# 但它依赖 V19 播种的四个场景资产（radar-001 与 P-HK01），所以排在 21 之后、10-scope 之前——
# 那些套件都会建/删临时数据，靠后放能让它的断言对着已知的演示数据，少一层噪声。
#
