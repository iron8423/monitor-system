...
- **用户管理**（管理端「用户」页签，仅 ADMIN）：列出全部账号（账号 / 姓名 / 角色 / 公司 / 岗位 / 电话 / 邮箱 / 启用），可**新建账号**（初始密码 ≥ 8 位）、**改任意用户的资料与角色**、**停用/启用**。停用与降权**立刻生效**（鉴权每请求回库核对），无需等令牌过期；接口不外泄 `password`/`tokenVersion`；带三条自锁保护——不能停用自己、不能改自己的角色、不能删自己。
...
| 验证 | 命令 | 本轮实测 |
|---|---|---|
| 后端验收（15 套件） | `tools/acceptance/run-all.sh --fresh` | **518 条断言 / 0 失败** |
