...
| 前端 | Vue 3.5 · Vite 6 · Pinia · Element Plus（含白天/黑夜双主题）· ECharts 6 · CesiumJS 1.145 |
...
- **使用说明页**（2026-09-18，所有角色，侧边栏最后一项）：以「你的角色」开头（落地页 / 可见菜单 / 可做的处置动作），随后按侧边栏顺序逐个说明模块功能与注意事项，末尾 7 条常见口径（时间、数据范围、告警判定、逻辑删除、权限边界、主题、数据性质声明）。
- **系统运维页**（2026-09-18，仅 ADMIN）：看服务本身而不是数据——进程/JVM/堆内存、数据库与连接池、SSE 连接数、Flyway 迁移清单（含每条的校验和与耗时）、关键表行数与测量时间跨度、以及会影响行为的运行时开关（严格契约/批大小/时钟容差/上传目录），密钥只回显「是否仍是默认值」。**全部只读**：备份、清理、重跑迁移仍留在命令行，因为那类动作的失败代价不该由一个 HTTP 接口承担。接口文档（Swagger）也从这一页直达。
...
- **白天 / 黑夜主题（2026-09-18）**：顶栏、登录页与大屏各有一个 **`☀ 白天 \| ☾ 黑夜` 手动切换按钮**（当前项实心高亮），选择记在本地；首次访问才跟随系统，之后以手动为准。工作台、Element Plus 组件与 3D 大屏 HUD 全跟着切，**只有影像与地形不动**（那是数据）。新增组件取色用 `--mk-*` 变量，见 `frontend/README.md` 的「主题」一节。
...
| 验证 | 命令 | 本轮实测 |
|---|---|---|
| 后端验收（15 套件） | `tools/acceptance/run-all.sh --fresh` | **532 条断言 / 0 失败** |
