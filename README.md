...
- **`<img>` 取影像是契约里第二处（也是最后一处）query 鉴权**（2026-09-14）：`<img>` 发不出
  ...
  错误明细就是 `取图 HTTP 401`。
- **影像模块没有删除端点**（契约 §7 只有上传 / 列表 / 内容三条），所以上传了就没有接口能撤回。
  两个后果：① 验收套件 `06-media.sh` 每跑一轮就往库里留一张 1×1 测试图，**回收不了**
  （与 2026-09-14 给告警套件补的 `recycle_point()` 不同，那批有 `DELETE /points` 可用）；
  ② 运维误传的现场照片没有任何界面手段可以删。**加删除端点属契约变更 → 已上报待用户定案**。
  演示库里的历史测试图已手工清掉（24 行 → 3 行，盘上文件同步核对过）。
